...
For the shortest **80 ms, batch-one chunk**, the exact small-matrix kernels give **7.070 ms encode / 6.247 ms decode**. A fresh matched comparison measures **6.58× / 5.89×** versus original eager and **1.46× / 1.46×** versus original CUDA graphs. This is a different input geometry from the 240 ms table above. [One-frame original/current comparison](results/full_codec_short_rows.json).
...
The CuTe residual combination passes all 15 corpus cases, and long incremental streams retain exact **11,072 tokens and 664,320 samples**. The full suite now passes **343 tests**. [CuTe fidelity](results/full_fidelity_ffn_cute.json), [streaming fidelity](results/full_incremental_ffn.json), [tests](results/tests.txt).
