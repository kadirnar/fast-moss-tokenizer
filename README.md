...
Three rotating rounds, 200 graph warmups and 30 timed calls per method/shape. Both paths use the same native precision. Graph timings include input/output copies. File I/O, resampling, model loading and setup are excluded. The graph adapter removes a host length synchronization for complete frames. [Measurements](results/v2_compare.json) · [Methodology](docs/v2.md).

**Validation:** **1,146 tests pass**. Tokens, encoder hidden states and stereo waveforms are bit-identical across 14 corpus cases, changed graph inputs and native streaming checks. See [offline results](results/v2_fidelity.json) and [streaming results](results/v2_streaming_b1.json). A 100× whole-model speedup has not been achieved.
