...
For the shortest **80 ms, batch-one chunk**, the exact small-matrix kernels give **7.064 ms encode / 6.241 ms decode**. A fresh matched comparison measures **6.62× / 5.92×** versus original eager and **1.46× / 1.46×** versus original CUDA graphs. This is a different input geometry from the 240 ms table above. [One-frame original/current comparison](results/full_codec_mid_one.json).
