...
A cuBLASLt pedantic-FP32 prototype selects measured algorithms and transposed FP32 weight layouts. At batch eight / 240 ms, combined encode/decode falls from **30.913 to 26.043 ms (1.19×)** against the already optimized graph. All 14 offline cases retain exact tokens, hidden states, and audio. An eight-lane 12.96-second stream with pauses, completion, and reuse also remains exact; filled-history latency improves **21.361 → 18.908 ms encode (1.13×)** and **19.364 → 16.856 ms decode (1.15×)**. The combined model needs an additional 5.69 GB of packed FP32 weights and peaks at 13.25 GB allocated. This version/hardware-specific experiment stays under `benchmarks/`, separate from the supported runtime. [Offline evidence](results/full_cublaslt_experiment.json), [streaming evidence](results/full_cublaslt_streaming.json), [324 matrix stress checks](results/cublaslt_fidelity.json).
