...
| --- | ---: | ---: | ---: | ---: |
| Encode | 48.168 ms | 11.910 ms | 7.677 ms | 6.27× |
| Decode | 38.454 ms | 10.168 ms | 6.347 ms | 6.06× |

Three rotating-order rounds use independent restored contexts and ten samples per mode/direction. Graph timings include input copies and owned outputs; loading, packing and capture are excluded. Tokens, hidden states, waveforms and restored outputs are exact. At batch eight, the measured graph latency is **9.994 ms encode / 8.618 ms decode**, or **5.02× / 4.65×** versus original eager at the same batch. Relative to the original graph, gains are **1.55× / 1.60×** at batch one and **1.92× / 1.99×** at batch eight. [240 ms matched comparison](results/full_codec_wide_cta_f3.json). The [earlier seeded-input comparison](results/full_compare_lane_reset_240ms.json) remains historical evidence. **100× whole-model acceleration has not been demonstrated.**

For the shortest **80 ms, batch-one chunk**, the exact kernels give **6.711 ms encode / 5.924 ms decode**. A fresh matched comparison measures **7.08× / 6.34×** versus original eager and **1.54× / 1.54×** versus original CUDA graphs. This is a different input geometry from the 240 ms table above. [One-frame original/current comparison, including long-K CUDA matrices](results/full_codec_wide_cta_f1.json).
