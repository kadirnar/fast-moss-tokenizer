...
| --- | ---: | ---: | ---: | ---: |
| Encode | 48.160 ms | 11.830 ms | 7.470 ms | 6.45× |
| Decode | 38.396 ms | 10.188 ms | 6.218 ms | 6.18× |

Three rotating-order rounds use independent restored contexts and ten samples per mode/direction. Graph timings include input copies and owned outputs; loading, packing and capture are excluded. Tokens, hidden states, waveforms and restored outputs are exact. At batch eight, the measured graph latency is **9.997 ms encode / 8.623 ms decode**, or **5.00× / 4.64×** versus original eager at the same batch. Relative to the original graph, gains are **1.58× / 1.64×** at batch one and **1.89× / 2.00×** at batch eight. [240 ms matched comparison](results/full_codec_norm_projection_f3.json). The [earlier seeded-input comparison](results/full_compare_lane_reset_240ms.json) remains historical evidence. **100× whole-model acceleration has not been demonstrated.**

For the shortest **80 ms, batch-one chunk**, the exact kernels give **6.586 ms encode / 5.835 ms decode**. A fresh matched comparison measures **7.17× / 6.44×** versus original eager and **1.57× / 1.56×** versus original CUDA graphs. This comparison uses 200 extra graph warmups per context and a different input geometry from the 240 ms table above. [Warmed one-frame comparison](results/full_codec_norm_projection_f1_warm.json). The [initial comparison](results/full_codec_norm_projection_f1.json) is retained; its first optimized encoder round showed a large timing transient.
...
A research attention-output/residual fusion covers nine native matrix shapes. After a singleton-stride correction, batch-one latency falls about **1.45% encode / 0.89% decode at 80 ms**, and **1.63% / 0.38% at 240 ms**. Both 48-case backend gates and long streams remain exact. Batch-eight one-frame decode has no established gain, and production integration remains pending. [Research evidence](docs/research.md#attention-output-projection-and-residual-fusion).
