...
| --- | ---: | ---: | ---: | ---: |
| Encode | 48.087 ms | 11.923 ms | 7.897 ms | 6.09× |
| Decode | 38.337 ms | 10.168 ms | 6.582 ms | 5.82× |

Three rotating-order rounds use independent restored contexts and ten samples per mode/direction. Graph timings include input copies and owned outputs; loading, packing and capture are excluded. Tokens, hidden states, waveforms and restored outputs are exact. At batch eight, the measured graph latency is **9.993 ms encode / 8.609 ms decode**, or **5.02× / 4.65×** versus original eager at the same batch. Relative to the original graph, gains are **1.51× / 1.54×** at batch one and **1.92× / 2.00×** at batch eight. [240 ms matched comparison](results/full_codec_strided_layer_norm_f3.json). The [earlier seeded-input comparison](results/full_compare_lane_reset_240ms.json) remains historical evidence. **100× whole-model acceleration has not been demonstrated.**

For the shortest **80 ms, batch-one chunk**, the exact kernels give **6.787 ms encode / 6.002 ms decode**. A fresh matched comparison measures **6.90× / 6.18×** versus original eager and **1.52× / 1.52×** versus original CUDA graphs. This is a different input geometry from the 240 ms table above. [One-frame original/current comparison, including strided CUDA LayerNorm](results/full_codec_strided_layer_norm_f1.json).
