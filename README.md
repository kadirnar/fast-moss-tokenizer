...
| --- | ---: | ---: | ---: | ---: |
| Encode | 47.839 ms | 11.812 ms | 7.464 ms | 6.41× |
| Decode | 38.265 ms | 10.167 ms | 6.212 ms | 6.16× |

Three rotating-order rounds use independent restored contexts and ten samples per mode/direction. Graph timings include input copies and owned outputs; loading, packing and capture are excluded. Tokens, hidden states, waveforms and restored outputs are exact. At batch eight, the measured graph latency is **9.988 ms encode / 8.619 ms decode**, or **5.03× / 4.65×** versus original eager at the same batch. Relative to the original graph, gains are **1.58× / 1.64×** at batch one and **1.91× / 2.00×** at batch eight. [240 ms matched comparison](results/full_codec_strided_ffn_f3.json). The [earlier seeded-input comparison](results/full_compare_lane_reset_240ms.json) remains historical evidence. **100× whole-model acceleration has not been demonstrated.**

For the shortest **80 ms, batch-one chunk**, the exact kernels give **6.626 ms encode / 5.862 ms decode**. A fresh matched comparison measures **7.12× / 6.39×** versus original eager and **1.56× / 1.55×** versus original CUDA graphs. This is a different input geometry from the 240 ms table above. [One-frame original/current comparison, including strided encoder FFN fusion](results/full_codec_strided_ffn_f1.json).
...
A research prototype also fuses one-row LayerNorm into QKV/GELU projections. It passes 48 full-model cases and shows **0.43% / 0.31% lower one-frame encode/decode latency** in a seven-round warmed comparison. It remains outside the supported runtime pending integration and its lifecycle/streaming gates. [Research results](docs/research.md#layernorm-inside-native-one-row-projections).
