...
| --- | ---: | ---: | ---: | ---: |
| Encode | 47.972 ms | 11.817 ms | 7.866 ms | 6.10× |
| Decode | 38.252 ms | 10.234 ms | 6.541 ms | 5.85× |

Three rotating-order rounds use independent restored contexts and ten samples per mode/direction. Graph timings include input copies and owned outputs; loading, packing and capture are excluded. Tokens, hidden states, waveforms and restored outputs are exact. At batch eight, the measured graph latency is **10.001 ms encode / 8.626 ms decode**, or **4.99× / 4.61×** versus original eager at the same batch. Relative to the original graph, gains are **1.50× / 1.56×** at batch one and **1.91× / 1.99×** at batch eight. [240 ms matched comparison](results/full_codec_cta_tiled_f3.json). The [earlier seeded-input comparison](results/full_compare_lane_reset_240ms.json) remains historical evidence. **100× whole-model acceleration has not been demonstrated.**

For the shortest **80 ms, batch-one chunk**, the exact kernels give **6.758 ms encode / 5.970 ms decode**. A fresh matched comparison measures **7.01× / 6.28×** versus original eager and **1.53× / 1.52×** versus original CUDA graphs. This is a different input geometry from the 240 ms table above. [One-frame original/current comparison, including CUDA matrix partitions](results/full_codec_cta_tiled_f1.json).
