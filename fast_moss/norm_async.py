"""Exact native-order norm/GEMV with asynchronous shared weight staging.
...
import hashlib
import torch
from .norm_projection import header,prepare
...
'''
CONFIGS={'none':(0,128,64,2,8,16,1),'gelu':(0,128,64,2,0,16,1)}
CACHE={}
...
def linear(x,w,g,b,eps,mode,config=None,debug=False,resources=False):
    ...
    expected=(3840,1280) if mode=='none' else (5120,1280) if mode=='gelu' else None
    if (x.shape!=(1,1280) or w.shape!=expected or g.shape!=(1280,) or b.shape!=(1280,)
