# Legacy v1 — 24 kHz mono

Historical v1 results and API. The current target is [48 kHz stereo v2](../README.md). These speedups do not describe v2.
...
This legacy page targets the original **24 kHz mono** checkpoint. The active **48 kHz stereo v2** implementation and measurements are in the [main README](../README.md).
...
**Validation:** 1,079 tests pass; full-checkpoint gates compare tokens, hidden states and waveform bits. Long streaming checks match corrected eager streaming. Streaming and offline decoding have an existing small rounding difference, documented in the [streaming reference](implementation.md#streaming). Experimental kernels are excluded from the table. A 100× whole-model speedup has not been achieved.
...
See the [streaming example](implementation.md#streaming) and [queued request example](implementation.md#queued-streaming-requests).
...
[Implementation and API details](implementation.md) · [Research notes](research.md) · [Development log](progress.md)
