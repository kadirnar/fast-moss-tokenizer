...
| --- | ---: | ---: | ---: | ---: |
| Encode | 47.555 ms | 11.806 ms | 7.357 ms | 6.46× |
| Decode | 37.921 ms | 10.170 ms | 6.184 ms | 6.13× |

Three rotating-order rounds use independent restored contexts, 200 extra graph warmups per context, and ten samples per mode/direction. Graph timings include input copies and owned outputs; loading, packing and capture are excluded. Tokens, hidden states, waveforms and restored outputs are exact. At batch eight, the measured graph latency is **10.022 ms encode / 8.675 ms decode**, or **4.97× / 4.58×** versus original eager at the same batch. Relative to the original graph, gains are **1.60× / 1.64×** at batch one and **1.91× / 1.98×** at batch eight. [240 ms matched comparison](../results/full_codec_norm_async_f3.json). The [earlier seeded-input comparison](../results/full_compare_lane_reset_240ms.json) remains historical evidence. **100× whole-model acceleration has not been demonstrated.**

For the shortest **80 ms, batch-one chunk**, the exact kernels give **6.455 ms encode / 5.744 ms decode**. A fresh matched comparison measures **7.24× / 6.44×** versus original eager and **1.60× / 1.58×** versus original CUDA graphs, with the same extra warmups as the 240 ms table. [Warmed one-frame comparison](../results/full_codec_norm_async_f1.json). These overall ratios are directly measured, not products of historical incremental gains.
...
Outstanding: broader audio corpus coverage, more streaming durations/batch schedules, network serving, multi-GPU execution, matrix-kernel optimization, and a defensible matched-workload 100× result. See [research notes](research.md) and [work log](progress.md).
...
The 48-case runtime ablation is bit-identical. At batch one / 80 ms, it improves encode **7.035 → 6.940 ms** and decode **6.210 → 6.078 ms**, about **1.4% / 2.2%**. At 240 ms, encoder timing is effectively unchanged and decode improves about 2%. These are isolated comparisons against the preceding optimized runtime, not whole-model speedup claims. [Runtime results](../results/full_native_layer_norm_runtime.json), [arithmetic and compiler investigation](research.md#native-welford-layernorm-with-constant-counts).

The CUDA backend now also reads validated transposed encoder inputs directly, eliminating their separate contiguous-copy launch. The isolated integrated comparison improves encoding by **1.1–2.6%** across batch-one/eight and 80/240 ms inputs; decoder timing stays within 0.09% of parity. All 48 full-model cases remain bit-identical. [Strided normalization ablation](../results/full_strided_layer_norm_runtime.json), [memory-layout investigation](research.md#eliminate-encoder-normalization-copies).
...
The initial five-shape extension passes its 48-case corpus bit for bit. Its paired batch-one graph measurements improve encode/decode latency by about **0.4% at 80 ms** and **0.7% at 240 ms**; the batch-eight control stays within 0.11% of parity. These modest whole-codec gains are separate from the larger component timings. [Integrated ablation](../results/full_cta_tiled_runtime.json), [implementation and selection evidence](research.md#single-block-native-order-matrix-partitions).

Six additional long-K shapes now use shared partials, covering all four small-matrix shapes that previously needed a separate reduction launch. The integrated 48-case comparison preserves all output bits and lowers batch-one graph latency by about **0.8% at 80 ms**, and **2.4% encode / 2.9% decode at 240 ms**. Batch-eight measurements remain within 0.09% of parity. The existing Triton backend remains available. [Integrated long-K ablation](../results/full_wide_cta_runtime.json), [search and repeated selection](research.md#long-k-matrices-without-global-partial-buffers).

## Supported asynchronous FFN contraction

The CUDA matrix backend with `ffn_backend="triton"` now uses native-order asynchronous shared-memory staging for one-row 1280×5120 FFN contractions. It keeps original FP32 weights and adds no persistent weight copy. The validated normalization/projection staging remains active. No new public option is required; the README quick start selects the path.

Warmup is tracked per module, shape, mode, stream and backend. Graphs remain inside the owning optimization context; packing and context exit invalidate them. Hooks, custom paths, gradients and autocast retain their existing fallbacks. The isolated codec gain is about **0.2–0.3%** at batch one / 80 ms. See [integration details](research.md#supported-asynchronous-ffn-contraction) and the [current speed table](../README.md#benchmark); earlier measurements in this reference remain historical.
