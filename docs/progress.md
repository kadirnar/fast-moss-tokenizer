...
## 2026-09-20 — Exact v2 pointwise fusion and final delivery

The preceding goal turn was **progress**: it delivered the verified native 48 kHz stereo v2 runtime at public commit `1535eac`. This continuation revalidated the clean worktree, public HEAD, and idle owned GPU workload, then used the recorded v2 profile to remove pointwise overhead. The user subsequently requested a readable, modular final project, an explicit end-to-end README, a public push and an end to further work. The final cleanup and validation below complete that delivery; no additional optimization campaign is started.

Two new Triton kernels fuse rotary rotation and scaled residual addition. BF16 values are promoted to FP32 before the original separate multiplications/additions, and RoPE outputs are converted back only at the original BF16 boundary. Native PyTorch sine/cosine and LayerNorm remain unchanged. The owner caches FP32 rotary frequencies, patches 12 rotary modules and 184 transformer layers, restores methods on exit, and retains native fallbacks for gradients, observed scale hooks and changed rotary periods. All model weights, quantizers and the native precision policy are preserved. The pinned upstream, PyTorch autocast and Triton compiler sources are linked in [the implementation notes](v2.md).

Three paired ablation rounds against the preceding cached runtime give **22.115 → 15.473 ms** at batch one / 80 ms (**1.4293× incremental speedup**) and **23.091 → 16.278 ms** at 240 ms (**1.4186×**). Batch-two gains are **1.4165× / 1.3989×**. A separate fresh original-eager / original-graph-adapter / optimized-graph comparison gives **139.262 → 15.492 ms, 8.99×** at batch one / 80 ms and **140.755 → 16.352 ms, 8.61×** at 240 ms. These are measured direct encode-to-decode calls, not products of historical ratios. Batch-two ratios are **8.92× / 8.00×**. The README's principal table is explicitly **End-to-End Benchmark (Encode → Decode)** and links the complete raw measurements.

All **14 full-checkpoint corpus cases** remain bit-identical for tokens, hidden states and waveforms, including changed graph inputs and restored methods. Both **162-chunk / 12.96-second** native stereo streaming checks pass at batch one/two, dispatching **29,808 fused rotary calls and 59,616 fused residual calls each**. The focused suites pass **11 existing v2 tests** and **22 new pointwise tests**, including BF16/FP32 layouts, QKV strides, subnormals, signed zeros, infinities, NaNs, offsets, hooks, gradients and graph replay. The final full suite passes **1,168 tests in 220.86 seconds**.

The refreshed profile records **14,402 original graph kernels versus 7,682 optimized kernels**, with **30.940 / 15.541 ms** aggregate kernel duration. The preceding cached profile had 13,030 kernels. Six representative compiled kernels contain separate FP32 multiply/add instructions with **no FMA or flush-to-zero instructions**, no spills and 0–1,024 bytes of shared memory. These resource and profile observations are separate from the headline wall-clock measurements.

The final code separates the stable public API (`v2.py`), pinned loader (`v2_loading.py`), codec adapters (`v2_codec.py`), context ownership (`v2_runtime.py`) and Triton kernels (`v2_pointwise.py`). V2 source, benchmarks and tests were consistently formatted. The module split happened after timing; **42 reproducible AST/byte checks** prove that every moved executable definition and all formatted numerical kernels retain the measured computation. The evidence embeds the original changed sources and hashes so the check is reproducible after checkout. The refreshed full-checkpoint profile and full repository suite execute the final modular layout. No speed result was silently relabeled as a new numerical implementation.

Wheel **0.2.1** includes all **39 runtime files**, matches source bytes and passes isolated imports. The final cross-report audit passes **267 checks**, including the module-equivalence proof, measured ratios, exact outputs, actual fused dispatch counts and README rows. [Ablation](../results/v2_pointwise_ablation.json) · [End-to-end comparison](../results/v2_pointwise_compare.json) · [Fidelity](../results/v2_pointwise_fidelity.json) · [Streaming one](../results/v2_pointwise_streaming_b1.json) · [Streaming two](../results/v2_pointwise_streaming_b2.json) · [Profile](../results/v2_pointwise_profile.json) · [Resources](../results/v2_pointwise_resources.json) · [Refactor proof](../results/v2_refactor_equivalence.json) · [Package](../results/v2_pointwise_package.json) · [Audit](../results/v2_pointwise_audit.json).

All owned GPU jobs are terminal: initial v2 regression `85104`, pointwise tests `17586`, full-checkpoint corpus `67380`, sequential ablation/comparison/streaming/profile/full-suite chain `83357`, and resource inspection `8062`. GPU work ran sequentially; the unrelated pre-existing process was untouched.

The delivered improvement is **progress**, not completion of the broader 100× objective. **100× remains unachieved. Further optimization is stopped at the user's explicit final-delivery request.**
