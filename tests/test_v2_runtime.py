        ...
        for group in (config.encoder_kwargs, config.decoder_kwargs):
            for block in group:
                if block["module_type"] == "Transformer":
                    block.update(
                        num_layers=1, d_model=64, dim_feedforward=128, num_heads=1
                    )
        return (
...
def test_v2_stereo_reference_graph_and_restore(model, batch, frames):
    x = torch.randn(batch, 2, frames * 3840, device="cuda") * 0.1
    ...
    with optimized(model) as owner:
        ...
        equal(ref, graph(x))
        equal(changed_ref, graph(changed))
        equal(ref, graph(x))
...
def test_v2_conv_parametrization_hooks_and_mutation(model):
    ...
    with optimized(model):
        equal((original(x),), (layer(x),))
        calls = []
        hook = layer.parametrizations.weight.register_forward_hook(
        ...
        try:
            ...
            hook.remove()
        next(layer.parametrizations.weight.parameters()).add_(0.01)
        equal((original(x),), (layer(x),))
...
@pytest.mark.parametrize(
    "shape", [(1, 1, 3840), (1, 2, 1920), (0, 2, 3840), (1, 2, 4000)]
)
