...
from fast_moss.cuda_matrices import CONFIGS,compiler,linear
...
def test_cuda_native_storage_graph_and_restoration(monkeypatch,shape):
    ...
    with optimized(model,matrix_backend='cuda'):
        ...
        exact(layer(x),ref);assert owner.cuda_calls==1 and owner.packed_bytes==0
        assert layer.weight.data_ptr()==pointer
        graph=GraphedCallable(lambda z:(layer(z),),x);exact(graph(x)[0],ref)
        before=owner.cuda_calls;owner.cuda_enabled=False;exact(layer(x),ref);assert owner.cuda_calls==before
...
def test_cuda_fallbacks_grad_autocast_alignment_custom_hook(monkeypatch):
    profile(monkeypatch)
    layer=torch.nn.Linear(768,768,bias=False).cuda().eval().requires_grad_(False);model=torch.nn.Sequential(layer).eval()
    x=torch.randn(4,768,device='cuda');original=layer.forward
    ...
    with optimized(model,matrix_backend='cuda'):
        ...
        assert owner.cuda_calls==1
    layer.forward=lambda z:original(z)+.25;custom=layer.forward;ref=layer(x)
    with optimized(model,matrix_backend='cuda'):
        exact(layer(x),ref);assert model._fast_matrix_runtime.cuda_calls==0
    assert layer.forward is custom


@torch.inference_mode()
@pytest.mark.parametrize('width,rows',[(768,4),(1280,1)])
@pytest.mark.parametrize('residual',['triton','cute'])
def test_cuda_coexists_with_ffn_and_norm(monkeypatch,width,rows,residual):
    profile(monkeypatch)
