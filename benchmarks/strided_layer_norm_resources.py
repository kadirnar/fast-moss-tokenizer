...
import torch
from fast_moss.normalization import _compile, _check, compiler
from fast_moss.strided_normalization import CONFIGS, SOURCE
...
def main():
    ...
    report = {'scope': 'integrated strided LayerNorm compiler resource audit', 'revision': REVISION,
              'torch': torch.__version__, 'gpu': torch.cuda.get_device_name(),
              'source_sha256': hashlib.sha256(SOURCE.encode()).hexdigest(), 'records': []}
    for n, time, config in sorted({(shape[2], shape[1], config) for shape, config in CONFIGS.items()}):
        _, fn = _compile(device, n, config, bindings, time)
        register, threads, unroll, stage, pad = config
        definitions = dict(N=n, REGISTER=int(register), THREADS=threads, UNROLL=unroll, T=time, STAGE=int(stage), PAD=pad)
        ...
        record = {'width': n, 'time': time, 'config': config, 'flags': [o.decode() for o in options],
    ...
    Path('results/strided_layer_norm_resources.json').write_text(json.dumps(report, indent=2)+'\n')
