...
def main():
    ...
    args = p.parse_args()
    model = load_model();clip,source = stereo_source()
    report = {'scope':'v2 original eager versus optimized native BF16 compute with FP32 parameters/quantizer; true stereo and independent-channel synthetic signals',
              **source_state(),'model_id':MODEL_ID,'revision':REVISION,'source':source,
              ...
              'gpu':torch.cuda.get_device_name(),'torch':torch.__version__,'cases':[],'streaming':args.streaming}
    ...
    if not args.streaming:
        for name,x in corpus(clip):
            ...
            with optimized(model) as owner:
                checks = {'eager':compare(ref,native(x)), 'fixed_adapter':compare(ref,codec(model,x))}
                ...
                counts = {k:getattr(owner,k) for k in ['linear_modules','conv_modules','quantizer_modules','cached_bytes','linear_calls','prepare_calls']}
                del graph
            checks['restored'] = compare(ref,native(x))
            exact = all(c['bits_equal'] for v in checks.values() for c in v)
            report['cases'].append({'name':name,'shape':list(x.shape),'checks':checks,'counts':counts,'all_exact':exact})
            save();print(name,exact,counts,flush=True)
            if not exact: raise SystemExit('V2 corpus mismatch')
    else:
        index = torch.arange(args.stream_frames*3840,device='cuda')[None]+torch.arange(args.batch,device='cuda')[:,None]*3840
        x = clip[:,index%clip.shape[1]].permute(1,0,2).contiguous()
        # Explicit cyclic extension of the 2.48-second real stereo recording.
        enc = model.encode(x,return_dict=True,chunk_duration=.08)
        ref = model.decode(enc.audio_codes,return_dict=True,chunk_duration=.08)
        print('native streaming complete',args.batch,args.stream_frames,flush=True)
        with optimized(model) as owner:
            actual_enc = model.encode(x,return_dict=True,chunk_duration=.08)
            actual_dec = model.decode(actual_enc.audio_codes,return_dict=True,chunk_duration=.08)
            checks = {'encode':compare((enc.audio_codes,enc.audio_codes_lengths,enc.encoder_hidden_states),
                                       (actual_enc.audio_codes,actual_enc.audio_codes_lengths,actual_enc.encoder_hidden_states)),
                      'decode':compare((ref.audio,ref.audio_lengths),(actual_dec.audio,actual_dec.audio_lengths))}
            counts = {k:getattr(owner,k) for k in ['cached_bytes','linear_calls','prepare_calls','select_calls']}
        exact = all(c['bits_equal'] for v in checks.values() for c in v)
        report.update(batch=args.batch,frames=args.stream_frames,seconds=args.stream_frames*.08,
                      input_transform='both stereo channels extended cyclically; lane i offset by i*3840 samples')
        report['cases'].append({'name':'native_streaming','shape':list(x.shape),'checks':checks,'counts':counts,'all_exact':exact})
        print('streaming exact',exact,counts,flush=True)
        if not exact:
            save();raise SystemExit('V2 streaming mismatch')
