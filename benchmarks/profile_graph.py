...
def kernel_summary(path,replays=5):
    ...
    small_names=['attention_residual','norm_gemv','_strided_ffn_fixed','strided_ffn_cuda','_short_fixed','short_ffn_cuda','wide_cta','cta_tiled','_small_gemv','_ffn_gemv','_small_fixed','_small_grouped','_small_parts','_small_reduce']
    ...
    return {'scope':'CUDA kernel events only, excludes memcpy and host time','replays':replays,
            ...
            'small_matrix_breakdown_scope':'Already included in groups.small_matrix; do not add again. Attention residual shares GEMV/fixed kernel names and adds its CUDA name; normalization/projection includes fused LayerNorm; FFN GEMV, short-row and strided FFN kernels include their fused epilogues.',
