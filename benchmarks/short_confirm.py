"""Stress exact 2/4/8-row candidates and compare warm/evicted component latency."""
import json
import statistics
...
from benchmarks.short_rows import run,compiled_resources
...
def main():
    ...
    search=json.loads(Path('results/short_rows.json').read_text())
    report={'scope':'2/4/8-row stress and three alternating warm/evicted rounds',
    ...
    for record in search['records']:
        ...
        for config in configs:
            ...
            fns[name]=lambda z,cfg=config:run(z,w,cfg)
            compiled[name]=compiled_resources(x,w,config)
        ...
        result={'shape':shape,'resources':compiled,'configs':configs,'checks':checks,'exact':exact,'rounds':rounds,'medians_ms':medians}
        report['records'].append(result)
        print(shape,'exact',exact,'medians',medians,flush=True)
        Path('results/short_confirm.json').write_text(json.dumps(report,indent=2)+'\n')
