"""Exact 2/4/8-row full-model gate and independent-context runtime ablation."""
import argparse
import json
import statistics
...
import torch
from benchmarks.short_rows import run as matrix_run
...
def candidate(model,configs):
    ...
    small.CONFIGS={**BASELINE,**(configs if RUNTIME else {})}
    ...
    for module in model.modules():
        ...
        def forward(module,x,*args,_original=original,**kwargs):
            ...
            if (shape in configs and not args and not kwargs and x.is_contiguous() and folds_to_mm(x)
                ...
                y=matrix_run(x.reshape(shape[0],shape[-1]),module.weight,config)
...
def main():
    ...
    parser.add_argument('--output',default='results/full_short_rows.json')
    parser.add_argument('--selection',choices=['warm','cold'],default='cold')
    args=parser.parse_args();RUNTIME=args.runtime
    baseline=json.loads(Path('results/short_baseline.json').read_text())
    ...
    confirmation=json.loads(Path('results/short_confirm.json').read_text())
    ...
    if RUNTIME:
        ...
        if small.CONFIGS!={**BASELINE,**configs}:
    ...
    report={'scope':'exact 2/4/8-row matrices: full-model fidelity and independently restored ablation',
            ...
            'previous_commit':'e5c974a', 'selection':args.selection, 'timing_scope':'three alternating independent-context rounds; five samples of ten graph calls, including owned outputs and input copies; excludes loading, capture and context setup'}
    def run(x):
    ...
    def inputs():
        ...
        for source,clip in zip(sources,clips):
            for batch,frames in [(2,1),(4,1),(8,1),(1,2),(1,3),(2,3),(1,6),(1,12)]:
    ...
    for name,value,source in inputs():
        ...
        del ref,x
    for batch,frames in ([] if args.fidelity_only else [(1,1),(2,1),(8,1),(1,3)]):
        idx=torch.arange(frames*1920,device='cuda')[None]+torch.arange(batch,device='cuda')[:,None]*1920
