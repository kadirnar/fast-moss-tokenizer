"""Research independent output chains per group for exact staged norm/GEMV.

Each output retains its original eight-thread cyclic FMA chain. Global-to-shared
...
__device__ __forceinline__ void copy_tile(float* dst,const float* W,int first,int start){
  constexpr int COLS=(THREADS/8)*OUTPUTS;
  #pragma unroll
...
extern "C" __global__ void norm_gemv_multi(const float* X,const float* W,
  ...
  for(int start=0;start<K;start+=TILE){
    ...
    for(int k=lane;k<TILE;k+=8){
      float value=normalized[start+k];
      #pragma unroll
      for(int o=0;o<OUTPUTS;++o)
        acc[o]=__fmaf_rn(value,weights[slot][(col+o*(THREADS/8))*(TILE+PAD)+k],acc[o]);
    }
    __syncthreads();
  }
  #pragma unroll
  for(int delta=4;delta;delta/=2){
    #pragma unroll
    for(int o=0;o<OUTPUTS;++o)
      acc[o]=__fadd_rn(acc[o],__shfl_xor_sync(0xffffffff,acc[o],delta,8));
  }
  if(lane==0){
    #pragma unroll
    for(int o=0;o<OUTPUTS;++o){
    float out=__fadd_rn(acc[o],0.f);
    #if MODE
    out=__fmul_rn(__fmul_rn(out,.5f),__fadd_rn(erff(__fmul_rn(out,.7071067811865476f)),1.f));
    #endif
    Y[first+col+o*(THREADS/8)]=out;
    }
  }
}
'''
...
def compile_kernel(n,mode,config,debug=False):
    register,threads,tile,stages,pad,unroll,asynchronous,outputs=config
    if (n not in (3840,5120) or mode not in ('none','gelu') or register not in (0,1)
        ...
        or tile not in (32,64,128,256,320,640) or stages not in (1,2) or pad not in (0,8)
        or unroll not in (1,4,16,32) or asynchronous not in (0,1)
        or outputs not in (1,2,4,8) or 5120+stages*(threads//8)*outputs*(tile+pad)*4+128>49152):
    ...
    defs=dict(N=n,K=1280,REGISTER_NORM=register,THREADS=threads,TILE=tile,STAGES=stages,PAD=pad,UNROLL=unroll,ASYNC=asynchronous,OUTPUTS=outputs,MODE=int(mode=='gelu'),DEBUG=int(debug))
    source=('\n'.join(f'#define {k} {v}' for k,v in defs.items())+'\n'+SOURCE).encode()
    cu,nvrtc=compiler();program=_check(nvrtc.nvrtcCreateProgram(source,b'norm_gemv_multi.cu',0,[],[]))
    try:
    ...
    module=_check(cu.cuModuleLoadData(blob));fn=_check(cu.cuModuleGetFunction(module,b'norm_gemv_multi'));a=cu.CUfunction_attribute
...
def linear(x,w,g,b,eps,mode,config,debug=False,resources=False):
    ...
    with torch.cuda.device(x.device):
        ...
        threads=config[1];cols=(threads//8)*config[7]
