"""Audit the integrated long-K single-CTA matrices compiler output with its production flags."""
import hashlib
import json
from pathlib import Path
import torch
from fast_moss.wide_matrices import CONFIGS, SOURCE, _compile, _check, compiler
...
def main():
    ...
    report = {'scope': 'integrated long-K single-CTA matrices compiler resource audit', 'revision': REVISION,
              'torch': torch.__version__, 'gpu': torch.cuda.get_device_name(),
              'source_sha256': hashlib.sha256(SOURCE.encode()).hexdigest(), 'records': []}
    for shape, config in sorted(CONFIGS.items()):
        _, fn = _compile(shape, config, bindings)
        m, n, k = shape
        ...
        record = {'shape': shape, 'config': config, 'flags': [o.decode() for o in options],
    ...
    Path('results/wide_cta_resources.json').write_text(json.dumps(report, indent=2)+'\n')
