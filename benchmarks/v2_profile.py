...
def main():
    ...
    report = {
        "scope": "batch-one 80 ms v2 direct encode/decode graphs; CUDA kernel durations under profiler, not headline wall latency",
        **source_state(),
        "model_id": MODEL_ID,
        "revision": REVISION,
        "source": source,
    ...
    for mode in ["original_graph_adapter", "optimized_graph"]:
        ...
        with optimized(model) if mode == "optimized_graph" else nullcontext():
            ...
            rows = [
                {
                    "name": name,
                    "calls_per_replay": count / 5,
                    "ms_per_replay": duration / 5000,
                    "percent_kernel_time": 100 * duration / total,
                }
                for name, (count, duration) in grouped.items()
            ]
            rows.sort(key=lambda row: row["ms_per_replay"], reverse=True)
            report["modes"][mode] = {
                "checks": checks,
                "kernel_count_per_replay": len(kernels) / 5,
                "kernel_ms_per_replay": total / 5000,
                "kernels": rows,
            }
            del graph
        print(mode, len(kernels) / 5, total / 5000, flush=True)
    report["all_exact"] = all(
        c["bits_equal"] for m in report["modes"].values() for c in m["checks"]
    )
