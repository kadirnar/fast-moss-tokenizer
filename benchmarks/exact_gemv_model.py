"""Exact cyclic GEMV full-model gate and independent-context runtime ablation."""
import argparse
import json
import statistics
...
def candidate(model,configs):
    ...
    small.CONFIGS={**BASELINE,**({s:('gemv',*c) for s,c in configs.items()} if RUNTIME else {})}
    ...
    for module in model.modules():
        ...
        def forward(module,x,*args,_original=original,**kwargs):
            ...
            if (shape in configs and not args and not kwargs and x.is_contiguous() and folds_to_mm(x)
                ...
                y=gemv(x.reshape(shape[0],shape[-1]),module.weight,*config,zero=True)
...
def main():
    ...
    parser.add_argument('--output',default='results/full_gemv.json')
    args=parser.parse_args();RUNTIME=args.runtime
    baseline=json.loads(Path('results/gemv_baseline.json').read_text())
    ...
    confirmation=json.loads(Path('results/gemv_confirm.json').read_text())
    ...
    configs=cold_configs
    if RUNTIME:
        if small.CONFIGS!={**BASELINE,**{s:('gemv',*c) for s,c in configs.items()}}:
            raise ValueError('Runtime table differs from selected candidates')
    backends=['previous','candidate']
    ...
    report={'scope':'exact cyclic one-row GEMV: full-model fidelity and independently restored ablation',
            ...
            'previous_commit':'f846998', 'timing_scope':'three alternating independent-context rounds; five samples of ten graph calls, including owned outputs and input copies; excludes loading, capture and context setup'}
    def run(x):
    ...
    for name,value,source in inputs():
        ...
        del ref,x
    for batch,frames in ([] if args.fidelity_only else [(1,1),(2,1),(1,3),(8,1)]):
        idx=torch.arange(frames*1920,device='cuda')[None]+torch.arange(batch,device='cuda')[:,None]*1920
        ...
        for repeat in range(3):
            for backend in (backends[repeat%len(backends):]+backends[:repeat%len(backends)]):
                ...
                active=configs if backend=='candidate' else {}
