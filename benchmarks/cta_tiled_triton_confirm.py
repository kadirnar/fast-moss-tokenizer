...
def main():
    ...
    sweep=json.loads(Path('results/cta_tiled_triton_tune.json').read_text())
    report={'scope':'Triton actual-weight stress and synthetic distinct-allocation rings; copies/capture excluded; no physical DRAM counters or codec performance claim',
    ...
    for row in sweep['records']:
        ...
        trials=sorted((t for t in row['trials'] if t['bits_equal'] and t['resources']['spills']==0),key=lambda t:t['ms'])
        configs=[];layouts=set()
        for trial in trials:
            layout=tuple(trial['config'][:3])
            if layout not in layouts:
                configs.append(tuple(trial['config']));layouts.add(layout)
        ...
        Path('results/cta_tiled_triton_confirm.json').write_text(json.dumps(report,indent=2)+'\n')
