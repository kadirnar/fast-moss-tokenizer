...
def stereo_source():
    ...
    if audio.shape[1] != 2:
        raise ValueError('Expected a real stereo source; channel averaging is not allowed')
    ...
    return value.cuda(), {
        ...
        'original_sampling_rate':rate, 'sampling_rate':48000, 'channels':audio.shape[1],
        'samples_per_channel':value.shape[1], 'transform':'resample both original stereo channels to 48 kHz; no channel averaging'}
...
def main():
    ...
    report = {'scope':'native 48 kHz stereo v2 eager encode/decode and direct codec roundtrip; resident input/model; excludes audio I/O and loading',
              'model_id':MODEL_ID, 'revision':REVISION, 'torch':torch.__version__,
              ...
              'sampling_rate':model.sampling_rate, 'channels':model.number_channels,
              'downsample_rate':model.downsample_rate, 'quantizers':32, 'source':source, 'cases':[]}
    for frames in args.frames:
        ...
        record = {'frames':frames,'batch':1,'input_shape':list(x.shape),
                  'codes_shape':list(enc.audio_codes.shape), 'audio_shape':list(dec.audio.shape),
                  'encode':measure(lambda:model._encode_frame(x), repeats=args.repeats),
                  'decode':measure(lambda:model._decode_frame(enc.audio_codes), repeats=args.repeats),
                  'roundtrip':measure(run,repeats=args.repeats)}
        report['cases'].append(record)
        print(frames,{k:record[k]['wall_ms_median'] for k in ('encode','decode','roundtrip')},flush=True)
