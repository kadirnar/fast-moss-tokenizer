...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--ring-ranking',action='store_true');args=parser.parse_args()
    ...
    probe=json.loads(Path('results/norm_gemv_async_ring.json' if args.ring_ranking else 'results/norm_gemv_async_probe.json').read_text())
    output=Path('results/norm_gemv_async_ring_confirm.json' if args.ring_ranking else 'results/norm_gemv_async_confirm.json')
    report={'scope':'research shared weight staging; five rotating rounds, fixed graphs, 200 warmups, nine samples of ten replays; copies/clones excluded; distinct allocations are synthetic cache pressure, not physical DRAM measurements',
        'previous_commit':'b40b3b4','ranking':'ring' if args.ring_ranking else 'warm','revision':REVISION,'source_sha256':hashlib.sha256(SOURCE.encode()).hexdigest(),'records':[]}
    for row in probe['records']:
        _,n,k=row['shape'];mode=row['mode'];w=matrices[(1,n,k)]['weight']
        ranked=(sorted([{'config':json.loads(name),'ms':v} for name,v in row['medians_ms'].items() if name!='current'],key=lambda r:r['ms']) if args.ring_ranking else sorted(row['trials'],key=lambda r:r['ms']));top=tuple(ranked[0]['config'])
        configs=list(dict.fromkeys([tuple(r['config']) for r in ranked[:2]]+[tuple(min((r for r in ranked if r['config'][3]==1),key=lambda r:r['ms'])['config']),(*top[:-1],0),(*top[:4],8-top[4],*top[5:]),(*top[:5],16,top[6])]))
