"""Exact 16/32/64-row full-model gate and independent-context runtime ablation."""
import argparse
import json
import statistics
...
def active_tables(configs):
    ...
    small.CONFIGS={**BASELINE,**{s:c for s,c in configs.items() if c[0]!='ordered'}}
    ...
    try:yield
    finally:small.CONFIGS,small.SHAPES,ordered.CONFIGS,ordered.SHAPES=previous
...
def main():
    ...
    parser.add_argument('--output',default='results/full_mid_rows.json')
    parser.add_argument('--selection',choices=['warm','cold'],default='cold')
    args=parser.parse_args();RUNTIME=args.runtime
    baseline=json.loads(Path('results/mid_baseline.json').read_text())
    ...
    confirmation=json.loads(Path('results/mid_confirm.json').read_text())
    configs={};cold_configs={}
    for record in confirmation['records']:
        prev=record['medians_ms']['native']
        viable=[(name,t) for name,t in record['medians_ms'].items() if name!='native'
    ...
    if RUNTIME:
        ...
        if (small.CONFIGS!={**BASELINE,**{s:c for s,c in configs.items() if c[0]!='ordered'}}
    ...
    report={'scope':'exact 16/32/64-row matrices: full-model fidelity and independently restored ablation',
            ...
            'previous_commit':'31b95eb', 'baseline_scope':'previous configuration table with corrected signed-zero arithmetic shared by both arms', 'selection':args.selection, 'timing_scope':'three alternating independent-context rounds; five samples of ten graph calls, including owned outputs and input copies; excludes loading, capture and context setup'}
    def run(x):
    ...
    def inputs():
        ...
        for source,clip in zip(sources,clips):
            for batch,frames in [(8,1),(4,2),(1,8),(1,3),(8,3),(2,3),(1,6),(1,12)]:
    ...
    for name,value,source in inputs():
        ...
        del ref,x
    for batch,frames in ([] if args.fidelity_only else [(8,1),(4,2),(1,8),(1,1),(8,3)]):
        idx=torch.arange(frames*1920,device='cuda')[None]+torch.arange(batch,device='cuda')[:,None]*1920
