...
def main():
    ...
    for frames in a.frames:
        codes=model.encode(torch.randn(1,1,1920*frames,device="cuda")*.05,return_dict=True).audio_codes
    ...
    report={"scope":"isolated actual-weight matrix operations, not whole-model speedups",
            ...
            "dtype":"float32","tf32":False,"triton_dot_precision":"ieee","records":[]}
    ...
    for shape,case in selected:
        ...
        for name,fn in candidates.items():
            ...
            record={"shape_MNK":shape,"layer":case["name"],"occurrences":case["calls"],"backend":name,
                    "reference_difference":difference(ref,out),"fp64_max_abs":(out.double()-gold).abs().max().item(),
