...
def main():
    ...
    p.add_argument(
        "--cached-ablation",
        action="store_true",
        help="Compare prior cached v2 runtime with new pointwise fusion",
    )
    args = p.parse_args()
    model = load_model()
    ...
    def fixed(x):
        return codec(model, x)

    modes = (
        ["cached_graph", "optimized_graph"]
        if args.cached_ablation
        else ["original_eager", "original_graph_adapter", "optimized_graph"]
    )
    report = {
        "scope": "48 kHz stereo v2 direct encode-to-decode; same native mixed precision; cached BF16 linear casts, FP32 quantizer and mixed-precision pointwise fusion",
        "cached_ablation": args.cached_ablation,
        **source_state(),
        "model_id": MODEL_ID,
        "revision": REVISION,
    ...
    for batch in args.batches:
        for frames in args.frames:
            ...
            record = {
                ...
                "audio_seconds": frames * 0.08,
                "adapter_checks": initial,
                "rounds": [],
            }
            for repeat in range(args.rounds):
                for mode in modes[repeat % len(modes) :] + modes[: repeat % len(modes)]:
                    ...
                    with (
                        optimized(model, pointwise=mode == "optimized_graph")
                        if mode in ("cached_graph", "optimized_graph")
                        else nullcontext()
                    ) as owner:
                        graph = (
                            None
                            if mode == "original_eager"
                            else GraphedCallable(fixed, x)
                        )
            ...
            record["medians_ms"] = {
                mode: statistics.median(
                    ...
                    if r["mode"] == mode
                    for v in r["timing"]["wall_ms_samples"]
                )
                for mode in modes
            }
            record["speedups"] = {
                mode: record["medians_ms"][mode]
                / record["medians_ms"]["optimized_graph"]
                for mode in modes[:-1]
            }
