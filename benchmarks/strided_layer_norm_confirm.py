...
def main():
    ...
    parser.add_argument('--input',default='results/strided_layer_norm_tune.json')
    parser.add_argument('--output',default='results/strided_layer_norm_confirm.json');args=parser.parse_args()
    strict_precision();torch.manual_seed(57139)
    cases=torch.load('results/strided_layer_norm_inputs.pt',weights_only=True)
    search=json.load(open(args.input))
    report={'scope':'actual strided LayerNorm finalists; native copy included; output/statistics checked bitwise',
    ...
    for row in search['records']:
        shape=tuple(row['shape']);r=cases[(shape,tuple(row['stride']))];x,g,b,eps=(r[k] for k in ('x','weight','bias','eps'))
        ...
        assert all(z.stride()==x.stride() for _,z in variants)
        checks=[]
        ...
        report['records'].append({'shape':shape,'stride':row['stride'],'configs':configs,'checks':checks,'exact':exact,'rounds':rounds,'medians_ms':medians})
