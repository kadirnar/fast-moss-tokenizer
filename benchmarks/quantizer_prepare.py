"""Research: pinned eight-channel LFQ normalization and distance preparation.
...
This module is not enabled by the supported runtime.
"""
import ctypes
import hashlib
...
def reference(x):
    enc = x.transpose(1, 2).reshape(-1, 8).float()
    denom = enc.norm(2, 1, keepdim=True).clamp_min(1.e-12)
    enc = enc / denom
    return enc, enc.pow(2).sum(1, keepdim=True), 2*enc, denom
