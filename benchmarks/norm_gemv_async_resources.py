...
import torch,triton
from benchmarks.norm_gemv_async import compile_kernel,SOURCE
...
def main():
    ...
    if args.runtime:
        from fast_moss.norm_async import compile_kernel as helper,SOURCE as runtime_source
    ...
    with tempfile.TemporaryDirectory(prefix='moss-async-sass-') as tmp:
        for row in prior['records']:
            for config in row['configs']:
                ...
                assert bool(async_copies)==bool(config[-1])
    ...
    report={'scope':'own-kernel SASS and byte-identical confirmation binaries','previous_commit':'b40b3b4','architecture':'sm_120',
        'source_sha256':hashlib.sha256(SOURCE.encode()).hexdigest(),'enabled_in_runtime':args.runtime,'records':records}
    Path('results/norm_async_resources.json' if args.runtime else 'results/norm_gemv_async_resources.json').write_text(json.dumps(report,indent=2)+'\n')
