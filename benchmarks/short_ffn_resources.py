"""Audit production short-FFN compiler resources, arithmetic and source provenance."""
import hashlib
import json
...
import triton
from fast_moss.short_ffn import CONFIGS,linear,cuda_source,compile_kernel,_short_fixed,_epilogue
...
def main():
    ...
    report={'scope':'production short FFN resources and captured arithmetic audit','revision':REVISION,'torch':torch.__version__,'triton':triton.__version__,
        'gpu':torch.cuda.get_device_name(),'library_sha256':hashlib.sha256(Path(library).read_bytes()).hexdigest(),
        'triton_source_sha256':{name:hashlib.sha256(fn.src.encode()).hexdigest() for name,fn in [('_short_fixed',_short_fixed),('_epilogue',_epilogue),('_mul',_mul),('_add',_add)]},
    ...
    for shape,(family,config) in sorted(CONFIGS.items()):
        m,n,k=shape;mode='gelu' if n>k else 'residual';x,w=cases[shape]['x'],cases[shape]['weight']
        residual=torch.randn(m,n,device='cuda');scale=torch.randn(n,device='cuda')*.01
        out,resources=linear(x,w,mode,residual,scale,library,bindings,True)
        raw=F.linear(x,w);ref=F.gelu(raw) if mode=='gelu' else residual+raw*scale
        assert bits(out,ref)
        record={'shape':shape,'family':family,'config':config,'mode':mode,'bits_equal':True,**resources}
        if family=='fixed':
            rows,cols,warps,u=config
            kernel=_short_fixed[(triton.cdiv(n,cols),triton.cdiv(m,rows))](x,w,residual,scale,out,m,n,k,rows,cols,u,mode,num_warps=warps,enable_fp_fusion=False,extern_libs={'libdevice':library})
            ...
            definitions=dict(M=m,N=n,K=k,ROWS=rows,COLS=cols,GROUPS=groups,UNROLL=u,DISTRIBUTE=int(distribute),MODE=int(mode=='residual'))
    ...
    Path('results/short_ffn_resources.json').write_text(json.dumps(report,indent=2)+'\n');print(report)
