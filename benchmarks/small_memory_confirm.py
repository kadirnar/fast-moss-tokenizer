...
def main():
    ...
    report={'scope':'explicit row accumulators and vector CUDA stress; three alternating warm/evicted component rounds',
    ...
    for shape,configs in selected.items():
        ...
        for name,config in configs:
            fns[f'{name}_{config}']=lambda z,name=name,config=config:(fixed(z,w,*config) if name=='fixed' else vector(z,w,tuple(config)))
        ...
        Path('results/small_memory_confirm.json').write_text(json.dumps(report,indent=2)+'\n')
